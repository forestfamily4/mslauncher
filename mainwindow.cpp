...
void MainWindow::timerEvent(QTimerEvent *event)
{
    ...
    if(this->scrolllate){
        QScrollBar* s=ui->scrollArea_2->verticalScrollBar();
        s->setValue(s->maximumHeight());
        scrolllate=false;
    }
    if(this->scrolllate2){
        QScrollBar* s=ui->scrollArea->verticalScrollBar();
        s->setValue(s->maximumHeight());
        scrolllate2=false;
    }
            ...
            if (stdoutFileSize != line.length())
            {
                ...
                else
                {
                    ...
                    if (line.contains("Done"))
                    {
                        ...
                        this->CommandLine_cloudflared->RunCloudflared(port);
                    }
                }
            }
...
void MainWindow::on_LaunchServerButton_clicked()
{
    ...
    if (!IsServerRunning)
    {
        ...
        ui->textEditcloudflaredlink->clear();
        Server *s = CurrentServer();
        this->CommandLine_Server = new CommandLineController();
        this->CommandLine_Server->RunServer(s);
...
void MainWindow::DataRender(bool isfirst)
{
    ...
    vector<Server> ss = this->Data.Servers;
    Server *s = new Server("", "", "", ServerType::official);
    if (!isfirst)
    {
        s = CurrentServer();
    }
...
void MainWindow::on_pushButton_OpenDirectory_clicked()
{
    ...
    const QFileInfo fileInfo(CurrentServer()->Directory);
